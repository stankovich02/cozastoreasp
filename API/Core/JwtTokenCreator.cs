...
namespace API.Core
{
    public class JwtTokenCreator
    {
        ...
        public JwtTokenCreator(CozaStoreContext context, JwtSettings settings, ITokenStorage storage)
        {
            _context = context;
            _settings = settings;
            _storage = storage;
        ...
        public string Create(string email, string password)
        {
            ...
            if (!BCrypt.Net.BCrypt.Verify(password + "CozaStore", user.Password))
            {
                throw new UnauthorizedAccessException();
            }
